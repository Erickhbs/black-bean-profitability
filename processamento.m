...
for(imagem=1:10)
    for(linha=1:5)
        for(coluna=1:5)
            im = imread(strcat('/home/phelyppe/Documentos/Workspace/black-bean-profitability/Crops/imagem',mat2str(imagem),',L',mat2str(linha),',C',mat2str(coluna),'.jpg'));
            imCinza = im;
            ...
            imwrite(imSegmentada,strcat('/home/phelyppe/Documentos/Workspace/black-bean-profitability/Imagem-Final/imagem',mat2str(imagem),',L',mat2str(linha),',C',mat2str(coluna),'.jpg'))
            
            % Mostrando porcentagem do processamento das imagens no terminal
            porcentagem = floor((contPercent)*100/250);
            clc
            disp(strcat(mat2str(porcentagem),'%'))
            contPercent++;
        end
    end
end